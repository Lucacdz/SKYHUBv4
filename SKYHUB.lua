...
-- Thêm các biến toàn cục ở đầu script
local fpsCounterEnabled = true -- Mặc định bật
local rainbowEffectCoroutine = nil
local fpsLabel = nil
local fpsFrame = nil
local screenGui = script.Parent -- Đảm bảo screenGui đã được khai báo

-- Hàm tạo FPS Counter (đã sửa lỗi)
local function createFPSCounter()
    -- Hủy frame cũ nếu tồn tại
    if fpsFrame then 
        fpsFrame:Destroy() 
        fpsFrame = nil
    end
    ...
    fpsFrame.Name = "RainbowFPSCounter"
    fpsFrame.Size = UDim2.new(0, 250, 0, 30)
    fpsFrame.Position = UDim2.new(1, -260, 0, 10)
    ...
    fpsFrame.Visible = fpsCounterEnabled
    ...
    fpsLabel.Text = "Sky Hub By Dungsky1337 | FPS: 0"
    fpsLabel.Font = Enum.Font.GothamBold
    fpsLabel.TextSize = 14
    fpsLabel.TextColor3 = Color3.new(1, 1, 1)
    fpsLabel.TextXAlignment = Enum.TextXAlignment.Left
    fpsLabel.Padding.Left = UDim.new(0, 10)
    ...
    local function updateFPS()
        if not fpsCounterEnabled or not fpsFrame or not fpsLabel then return end
        ...
        if elapsed >= 0.5 then
            ...
            if fpsLabel and fpsLabel.Parent then
                fpsLabel.Text = "Sky Hub By Dungsky1337 | FPS: "..tostring(currentFPS)
            end
        end
    end

    -- Hàm hiệu ứng cầu vồng
    local function rainbowEffect()
        local hue = 0
        while fpsCounterEnabled and fpsLabel and fpsLabel.Parent do
            hue = (hue + 0.01) % 1
    ...
    renderConnection = game:GetService("RunService").RenderStepped:Connect(function()
        pcall(updateFPS) -- Bọc trong pcall để bắt lỗi
    end)

    -- Bắt đầu hiệu ứng nếu được bật
    if fpsCounterEnabled then
        rainbowEffectCoroutine = coroutine.wrap(rainbowEffect)
        rainbowEffectCoroutine()
    end

    -- Hủy kết nối khi frame bị hủy
    fpsFrame.Destroying:Connect(function()
...
(function()
    ...
    if fpsCounterEnabled then
        ...
        if rainbowEffectCoroutine then
            rainbowEffectCoroutine = nil
        end
    end
end)
...
-- Nút tăng tốc độ
local speedUpButton = createStandardButton(MainTab, "+ Speed", 260)
speedUpButton.Size = UDim2.new(0, 120, 0, 30)

-- Nút giảm tốc độ
local speedDownButton = createStandardButton(MainTab, "- Speed", 260)
speedDownButton.Position = UDim2.new(0, 130, 0, 260)
speedDownButton.Size = UDim2.new(0, 120, 0, 30)
